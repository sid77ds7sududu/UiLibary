...
function Library:CreateLoadingScreen(Info)
    Info = Info or {}
    local Title    = tostring(Info.Title    or "Loading")
    local Subtitle = tostring(Info.Subtitle or "Please wait...")
    local Credits  = tostring(Info.Credits  or "UI Library by Xeioa")
    local Duration = tonumber(Info.Duration)

    local Gui = Instance.new("ScreenGui")
    Gui.Name = "ObsidianLoadingScreen"
    ...
    Overlay.BackgroundColor3 = Color3.fromRGB(8, 6, 18)
    ...
    CardGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(90, 55, 200)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(30, 20, 70)),
    ...
    CardStroke.Color = Color3.fromRGB(120, 80, 220)
    ...
    SubLabel.TextColor3 = Color3.fromRGB(180, 160, 255)
    ...
    BarBg.BackgroundColor3 = Color3.fromRGB(30, 22, 55)
    BarBg.Position = UDim2.new(0.5, 0, 0, 106)
    ...
    BarFill.BackgroundColor3 = Color3.fromRGB(130, 90, 240)
    ...
    BarGlow.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(1, Color3.fromRGB(90, 55, 200)),
    })
    BarGlow.Parent = BarFill

    local CreditsLabel = Instance.new("TextLabel")
    CreditsLabel.BackgroundTransparency = 1
    CreditsLabel.Position = UDim2.new(0, 0, 1, -36)
    CreditsLabel.Size = UDim2.new(1, 0, 0, 20)
    CreditsLabel.Text = Credits
    CreditsLabel.TextColor3 = Color3.fromRGB(120, 100, 180)
    ...
    local function AnimateBar(target, speed)
        local start = Progress
        local startTime = tick()
        local dur = speed or 0.4
        RunService:BindToRenderStep("ObsidianLoadBar", 1, function()
            local t = math.clamp((tick() - startTime) / dur, 0, 1)
            local eased = 1 - (1 - t) ^ 3
            local val = start + (target - start) * eased
            BarFill.Size = UDim2.fromScale(val, 1)
            if t >= 1 then
                Progress = target
                pcall(function() RunService:UnbindFromRenderStep("ObsidianLoadBar") end)
            end
        end)
    end
    ...
    function Data:UpdateProgress(value)
        AnimateBar(math.clamp(value, 0, 1))
    end

    function Data:SetStatus(text)
        SubLabel.Text = tostring(text)
    end

    function Data:Dismiss()
        if Dismissed then return end
        Dismissed = true
        AnimateBar(1, 0.3)
        task.delay(0.4, function()
            local ti = TweenService:Create(Overlay, TweenInfo.new(0.5, Enum.EasingStyle.Quad), { BackgroundTransparency = 1 })
            local tc = TweenService:Create(Card, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
    ...
    if Duration then
        ...
        task.spawn(function()
            for i = 1, steps do
                ...
                task.wait(stepTime)
                AnimateBar(i / steps, stepTime * 0.9)
            end
...
function Library:CreateWindow(WindowInfo)
    ...
    if Library.IsMobile then
        local function MakeMobileIconButton(icon, yOffset)
            local Btn = New("Frame", {
                AnchorPoint = Vector2.new(0, 0),
                BackgroundColor3 = Color3.fromRGB(18, 16, 28),
                Position = UDim2.fromOffset(6, yOffset),
            ...
            New("UIGradient", {
                Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, Color3.fromRGB(110, 70, 220)),
            ...
            New("UIStroke", {
                Color = Color3.fromRGB(140, 100, 240),
                Thickness = 1.2,
                Transparency = 0.25,
                Parent = Btn,
            })
            ...
            local ClickDetect = New("TextButton", {
                BackgroundTransparency = 1,
                Size = UDim2.fromScale(1, 1),
                Text = "",
                ZIndex = 12,
            ...
            Library:MakeDraggable(ClickDetect, Btn, true)

            return Btn, IconLabel, ClickDetect
        end

        local ToggleBtn, ToggleIcon, ToggleClick = MakeMobileIconButton("≡", 6)
        local LockBtn, LockIcon, LockClick = MakeMobileIconButton("o", 58)

        task.defer(function()
            ToggleIcon.Text = Library.Toggled and "x" or "="
            LockIcon.Text = Library.CantDragForced and "L" or "o"
        end)
        ...
        LockClick.MouseButton1Click:Connect(function()
        ...
        if WindowInfo.MobileButtonsSide == "Right" then
            ...
            LockBtn.Position = UDim2.new(1, -50, 0, 58)
            LockBtn.AnchorPoint = Vector2.new(1, 0)
        else
            LockBtn.Position = UDim2.fromOffset(6, 58)
        end
    end
